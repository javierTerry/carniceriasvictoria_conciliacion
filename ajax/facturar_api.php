...
if ($http_code_envio == 200) {
    ...
    if ($conexion_gen && !empty($uuid)) {
        ...
        try {
            ...
            error_log("[" . date('Y-m-d H:i:s') . "] 174 ----------", 3, $log_file);
        } catch (\Throwable $th) {
            //throw $th;
        ...
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "ssssissssis", 
                $mov_id, $uuid, $totalGlobal, $metodo_pago, $user_id, 
                $serie, $folio, $link_xml, $link_pdf, $cust_id, $branch
            );
    ...
    echo json_encode(array(
        ...
        'data' => array(
            'serie' => $serie, 'folio' => $folio, 'xml' => $link_xml, 'pdf' => $link_pdf, 'uuid' => $uuid
        )
    ));
} else {
    echo json_encode(array(
        'success' => false, 'message' => 'Error al enviar XML a SINUBE.', 'details' => $response_envio
    ));
}
?>
